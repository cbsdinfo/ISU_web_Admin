<!--
 * @description: 資源管理，用於為三方站點提供資源授權服務
 * @author: liyubao | xufuxing
 * @version: 1.0
 * @updateDate:2021-09-23 使用通用組件改造
-->
      ...
      <el-dialog v-el-drag-dialog :destroy-on-close="true" class="dialog-mini rule-form-dialog" width="850px" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
        <el-form :rules="rules" ref="dataForm" :model="temp" label-position="right" label-width="100px">
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item label-width="150px" size="small" :label="'資源標識（模塊編號）'" prop="sourceCode">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$api.dataprivilegerules.getList(this.listQuery).then((response) => {
    ...
    createData() {
      ...
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          this.$api.dataprivilegerules.add(this.temp).then(() => {
    ...
    updateData() {
      ...
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          ...
          this.$api.dataprivilegerules.update(tempData).then(() => {
