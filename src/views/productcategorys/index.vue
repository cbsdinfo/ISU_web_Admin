    ...
    <div class="app-container flex-item">
      <div class="bg-white" style="height: 100%">
        <el-table ref="mainTable" :key="tableKey" :data="list" v-loading="listLoading" border fit highlight-current-row style="width: 100%" height="calc(100% - 60px)" @row-click="rowClick" @selection-change="handleSelectionChange">
          ...
          <el-table-column min-width="50px" label="商品類別名稱" prop="name"></el-table-column>
          <el-table-column min-width="50px" label="商品類別圖檔" prop="picture"></el-table-column>
    ...
    <el-dialog v-el-drag-dialog class="dialog-mini" width="500px" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form label-width="100px" :model="temp" :rules="rules" ref="ruleForm">
        <el-row :gutter="8">
          ...
          <el-col :span="24">
            <el-form-item label="商品類別圖檔" prop="picture">
              <el-input type="text" v-model="temp.picture" size="small" placeholder="請輸入商品類別圖檔"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="24">
  ...
  methods: {
    ...
    createData() {
      ...
      this.$refs["ruleForm"].validate((valid) => {
        if (valid) {
          this.$api.productCategorys.add(this.temp).then(() => {
    ...
    updateData() {
      ...
      this.$refs["ruleForm"].validate((valid) => {
        if (valid) {
          ...
          this.$api.productCategorys.update(tempData).then(() => {
    ...
    handleDelete(rows) {
      // 多行刪除
      this.delrows("productCategorys", rows);
    },
  },
};
</script>
