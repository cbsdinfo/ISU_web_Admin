<template>
  <div style="height: 100%;">
    <el-container class="container">
      <el-aside width="300px" theme="light" class="select-area" v-if="!isShowContent">
        <el-row>
          <el-checkbox-button v-model="tag.checked0" class="tag">工具</el-checkbox-button>
          <div align="center" v-if="tag.checked0">
            <el-button-group>
              <el-button v-for="(tool, index) in field.tools" :key="index" size="mini" :icon="tool.icon" :type="currentTool.type == tool.type ? 'primary': 'default'" @click="selectTool(tool.type)" style="font-size: 16px;width: 100px;">
              </el-button>
            </el-button-group>
          </div>
        </el-row>
        <el-row>
          <el-checkbox-button v-model="tag.checked1" class="tag">基礎節點</el-checkbox-button>
          <div align="center" v-if="tag.checked1">
            <draggable tag="el-row" :list="field.commonNodes" v-bind="{group:{ name:'flow', pull:'clone',put:false},sort:false}" @end="handleMoveEnd" @start="handleMoveStart" :move="handleMove">
        ...
        <el-row>
          <el-checkbox-button v-model="tag.checked3" class="tag">泳道節點</el-checkbox-button>
          <div align="center">
            <draggable tag="el-row" :list="field.laneNodes" :grid="{ gutter: 8, column: 2 }" v-if="tag.checked3" v-bind="{group:{ name:'flow', pull:'clone',put:false},sort:false}" @end="handleMoveEnd" @start="handleMoveStart" :move="handleMove">
      ...
      <el-main style="padding:0;">
        <el-container style="height: 100%;">
          <el-header class="header-option" style="height: auto;" v-if="!isShowContent">
            <el-popover title="確認要重新繪製嗎？" placement="bottom" v-model="isShowPopover">
              <div style="text-align: center;">
                <el-button size="small" @click="isShowPopover = false">取消</el-button>
                <el-button type="primary" size="small" @click="clear">確認</el-button>
              </div>
              <el-tooltip slot="reference" content="重新繪製" placement="bottom">
                <el-button class="header-option-button" size="small" icon="el-icon-refresh-left"></el-button>
              </el-tooltip>
            </el-popover>
          </el-header>
          <el-main class="content" style="padding: 0;">
            <flow-area v-if="loadFlowArea && flowData" ref="flowArea" :browserType="browserType" :flowData="flowData" :select.sync="currentSelect" :selectGroup.sync="currentSelectGroup" :plumb="plumb" :isShowContent="isShowContent" :currentTool="currentTool" :isDrag="isDrag" @findNodeConfig="findNodeConfig" @selectTool="selectTool" @getShortcut="getShortcut" @saveFlow="saveFlow">
            </flow-area>
      ...
      <el-aside width="350px" theme="light" class="attr-area" @mousedown.stop="loseShortcut" v-if="!isShowContent">
        <flow-attr :plumb="plumb" :flowData="flowData" :formTemplate="formTemplate"></flow-attr>
      </el-aside>
    </el-container>
    <el-dialog :title="'流程設計圖_' + flowData.attr.id + '.png'" centered width="90%" :closable="flowPicture.closable" :maskClosable="flowPicture.maskClosable" :visible="flowPicture.modalVisible" okText="下載到本地" cancelText="取消" @ok="downLoadFlowPicture" @cancel="cancelDownLoadFlowPicture">
      <div align="center">
  ...
  methods: {
    ...
    getBrowserType() {
      ...
      if (
        ...
        alert("IE瀏覽器支持性較差，推薦使用Firefox或Chrome");
        return 5;
      }
      if (userAgent.indexOf("Trident") > -1) {
        alert("Edge瀏覽器支持性較差，推薦使用Firefox或Chrome");
        return 6;
      }
    },
    dealCompatibility() {
    ...
    initJsPlumb() {
      ...
      this.plumb.ready(() => {
        that.plumb.bind("beforeDrop", function (info) {
          ...
          const filter = that.flowData.lines.filter(
            (link) => link.from === from && link.to === to
          );
          if (filter.length > 0) {
            that.$message.error("同方向的兩節點連線只能有一條！");
            return false;
          }
          return true;
        });
        ...
        that.plumb.bind("connection", (conn) => {
          ...
          document
            ...
            .addEventListener("contextmenu", function (e) {
              ...
              const currentSelect = that.flowData.lines.filter(
                (l) => l.id === id
              )[0];
    ...
    saveFlow() {
      ...
      this.$message.success("保存流程成功！請查看控制台。");
      return d;
    },
    // toImage() {
    ...
    downLoadFlowPicture() {
      ...
      alink.download = "流程設計圖_" + that.flowData.attr.id + ".png";
      alink.click();
    },
    ...
    deleteLink() {
      ...
      lines.splice(
        lines.findIndex((link) => link.from === from && link.to === to),
        1
      );
