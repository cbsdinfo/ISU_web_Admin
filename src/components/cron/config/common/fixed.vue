<template>
  <div class="cell-div">
    <el-radio v-model="type_" :label="label" @change="change">
      ...
      {{ $t('common.specified') }}
      <el-select v-model="numArray" :collapse-tags="collapsed" :size="size" :placeholder="$t('common.placeholderMulti')" filterable multiple style="width: 100%;">
    ...
    tag_: {
      ...
      set(newValue) {
        ...
        arr.forEach((num) => {
          if (
            !isNumber(num) ||
            parseInt(num) < this.nums[0].value ||
            parseInt(num) > this.nums[this.nums.length - 1].value
          ) {
