<template>
  <div class="cell-div">
    <el-radio v-model="type_" :label="label" @change="change">
      ...
      <el-select v-model="lastNum" :size="size" :placeholder="$t('common.placeholder')" filterable>
...
export default {
  ...
  computed: {
    tag_: {
      get() {
        return (
          (this.lastNum >= 1 && this.lastNum < 7 ? this.lastNum : "") + LAST
        );
      },
      set(newValue) {
        if (this.type_ !== LAST) {
        ...
        if (
          !isNumber(numStr) ||
          parseInt(numStr) < this.nums[0].value ||
          parseInt(numStr) > this.nums[this.nums.length - 1].value
        ) {
